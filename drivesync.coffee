...
secretFile = path.join __dirname, 'client_secrets.json'
authCacheFile = path.join __dirname, '.auth.json'
  ...
  readSecretFile = (callback) ->
    fs.readFile secretFile, 'utf8', (error, data) ->
      if error
        ...
        fs.readFile authCacheFile, 'utf8', (error, authCacheData) ->
          authCache = null
          if error
